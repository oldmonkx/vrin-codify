...
export default function ConstructionUpdates() {
  ...
  return (
    /* Continues the cream bg-brand-cream + starry pattern from Location above */
    <section
    ...
    >
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        {/* Video + Playlist — compact horizontal layout */}
          ...
          >
            ...
            <div className="border border-brand-gold/20 overflow-hidden flex-1">
              {videos.map((video, index) => (
                ...
                >
                  <span className={`flex-shrink-0 mt-0.5 transition-colors duration-150 ${
                    activeIndex === index ? 'text-brand-gold-deep' : 'text-brand-paper/30 group-hover:text-brand-gold-deep'
                  }`}>
                    <Play size={12} fill="currentColor" />
                  </span>
                  <div className="min-w-0">
                    <p className={`text-[12px] leading-snug font-sans line-clamp-2 transition-colors duration-150 ${
                      activeIndex === index ? 'text-brand-gold-deep font-medium' : 'text-brand-paper/70 group-hover:text-brand-paper'
                    }`}>
                      {video.title}
                    </p>
